<template>
  <v-container fluid>
    ...
    <v-container v-else>
    <v-row>
      <v-col cols="12" sm="4" v-for="item in items.suspected_issues.summary_items" :key="item.title">
        <SummaryCard
      ...
      <v-container>
        <p>※ 上記は 「 <a :href="items.suspected_issues.source_info.url">{{ items.suspected_issues.source_info.date }}: {{ items.suspected_issues.source_info.description }}</a> 」までのデータを用いて算出しています。</p>
      </v-container>
    </v-row>
